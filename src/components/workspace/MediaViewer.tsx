...
export function MediaViewer({ title }: { title: string }) {
  ...
  function seek(offset: number) {
    const video = videoRef.current;
    if (video) video.currentTime = Math.max(0, video.currentTime + offset);
  }
    ...
    <Card className="relative min-h-0 overflow-hidden bg-slate-950 p-0">
      ...
      <div className="absolute inset-x-0 top-0 flex items-start justify-between p-4 text-white">
        <div>
          ...
          <p className="text-xs text-white/60">Lecture 1 • Video 4</p>
        </div>
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-4 pb-4 pt-10 text-white">
        <div className="flex items-center gap-2">
          ...
          <IconButton label="Skip back 10 seconds" onClick={() => seek(-10)} className="text-white">
            <RotateCcw size={16} />
          </IconButton>
          <IconButton
          ...
          <span className="text-[10px] text-white/70">06:42 / 24:18</span>
          <input
